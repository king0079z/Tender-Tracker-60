...
export default function TimelineUpdateModal({ 
  ...
  const [isManualRfiDue, setIsManualRfiDue] = React.useState(false);

  React.useEffect(() => {
    setFormData(timeline);
    // Check if RFI due date is different from the default calculation
    if (timeline.rfiSent.date && timeline.rfiDue.date) {
      const defaultDueDate = addDays(new Date(timeline.rfiSent.date), RFI_RESPONSE_DAYS);
      setIsManualRfiDue(defaultDueDate.getTime() !== new Date(timeline.rfiDue.date).getTime());
    }
  }, [timeline]);
  ...
  const updateMilestone = (key: keyof CompanyTimeline, data: Partial<Milestone>) => {
    setFormData(prev => {
      ...
      // Only auto-update RFI due date if manual mode is not enabled
      if (key === 'rfiSent' && data.isCompleted && newData.rfiSent.date && !isManualRfiDue) {
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    <input
                      ...
                      disabled={isRfiDue && !canEditRfiDue && !isManualRfiDue}
                      min="2024-01-01"
                      max="2026-12-31"
                      className={`w-full border rounded-md px-3 py-2 ${
                        isRfiDue && !canEditRfiDue && !isManualRfiDue ? 'bg-gray-100' : ''
                      }`}
                    />
                    {isRfiDue && canEditRfiDue && (
                      <div className="mt-2">
                        <label className="flex items-center">
                          <input
                            type="checkbox"
                            checked={isManualRfiDue}
                            onChange={(e) => {
                              setIsManualRfiDue(e.target.checked);
                              if (!e.target.checked && formData.rfiSent.date) {
                                // Reset to default due date
                                const defaultDueDate = addDays(new Date(formData.rfiSent.date), RFI_RESPONSE_DAYS);
                                updateMilestone('rfiDue', { date: defaultDueDate });
                              }
                            }}
                    ...
                    {isRfiDue && (
                      <p className="text-xs text-gray-500 mt-1">
                        {!canEditRfiDue 
                          ...
                          : isManualRfiDue
                          ? 'Custom due date enabled'
                          : `Default due date (${RFI_RESPONSE_DAYS} days after RFI sent)`}
                      </p>
                    )}
                  </div>
                  <div>
